...
YUI.add('moodle-enrol_stoa-quickenrolment', function(Y) {
    ...
    Y.extend(USERENROLLER, Y.Base, {
        ...
        initializer : function(config) {
            ...
            this.set(UEP.BASE, create('<div class="'+CSS.PANEL+' '+CSS.HIDDEN+'"></div>')
                .append(create('<div class="'+CSS.WRAP+'"></div>')
                    .append(create('<div class="'+CSS.HEADER+' header"></div>')
                        .append(create('<div class="'+CSS.CLOSE+'">x</div>'))
                    ...
                    .append(create('<div class="'+CSS.CONTENT+'"></div>')
                        .append(create('<div class="'+CSS.SEARCHCONTROLS+'"></div>')
                            .append(create('<div class="'+CSS.ENROLMENTOPTION+' '+CSS.ROLE+'"><label for="id_enrol_stoa_assignable_roles">'+M.str.role.assignroles+'</label></div>')
                                    .append(create('<select id="id_enrol_stoa_assignable_roles"><option value="">'+M.str.enrol.none+'</option></select>'))
                            )
            ...
            Y.all('.enrol_stoa_plugin input').each(function(node) {
        ...
        populateAssignableRoles : function() {
            this.on('assignablerolesloaded', function() {
                ...
                Y.one('#id_enrol_stoa_assignable_roles').focus();
            }, this);
        ...
        getAssignableRoles : function(){
            Y.io(M.cfg.wwwroot+'/enrol/stoa/ajax.php', {
                method:'POST',
        ...
        show : function(e) {
            ...
            var rolesselect = Y.one('#id_enrol_stoa_assignable_roles');
            if (rolesselect) {
                rolesselect.focus();
            }
        },
    ...
    M.enrol_stoa = M.enrol_stoa || {};
    M.enrol_stoa.quickenrolment = {
        init : function(cfg) {
   ...
   {requires:['base','node', 'overlay', 'io-base', 'test', 'json-parse',
              'event-delegate', 'dd-plugin', 'event-key', 'moodle-enrol-notification']});
